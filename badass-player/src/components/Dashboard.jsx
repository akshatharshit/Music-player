...
export default function Dashboard() {
  ...
  // --- REDUX STATE (Removed Auth) ---
  const { tracks = [], loading: tracksLoading } = useSelector((state) => state.audio);
  const { libraries = [], loading: libLoading } = useSelector((state) => state.library);
  
  // Hardcoded User for No-Login Mode
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-24 font-sans overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* RIGHT COLUMN (1/3 Width) - TOP CHARTS */}
